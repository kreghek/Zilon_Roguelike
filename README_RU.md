...
1. *Не обязательно*. **В переменной окружения `ZILON_LIV_SCHEME_CATALOG` указать полный путь до `\Zilon.Client\Assets\Resources\Schemes`**.
...
2. *Не обязательно*. **Установить расширение Specflow for Visual Studio 2019**.
...
3. *Не обязательно*.**Установить расширение ResX**.
...
4. **Теперь игру можно запустить из Visual Studio или из директории `bin` после сборки**.
Вы великолепны.

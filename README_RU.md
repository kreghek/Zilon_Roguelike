...
Игра требует наличия схем в папке `bin`. Самый простой способ разместить каталог в правильную директорию - это выполнить скрипт `copy_scheme_catalog_to_game_bin.bat` в корне репозитория. Этим же скриптом можно актуализировать схемы, если они изменились.

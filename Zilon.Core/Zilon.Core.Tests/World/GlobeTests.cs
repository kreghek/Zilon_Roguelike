    ...
    public class GlobeTests
    {
        ...
        public async Task UpdateAsyncTest_SectorFiresTransition_GlobeTransitionHandlersInitiatesTransitions()
        {
            ...
            taskSourceMock.Setup(x => x.GetActorTaskAsync(It.IsAny<IActor>(), It.IsAny<ISectorTaskSourceContext>()))
                .Returns<IActor, ISectorTaskSourceContext>((actor, context) => Task.FromResult((IActorTask)new SectorTransitTask(actor, taskContext)));
            ...
            actorMock.Verify(x => x.MoveToOtherSector(It.Is<ISector>(s => s == sector), It.IsAny<SectorTransition>()), Times.Once);
        }
    }
}
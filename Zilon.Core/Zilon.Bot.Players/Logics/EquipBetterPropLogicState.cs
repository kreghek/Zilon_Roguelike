...
namespace Zilon.Bot.Players.Logics
{
    public sealed class EquipBetterPropLogicState : LogicStateBase
    {
            ...
            ILogicStrategyData strategyData)
        {
            ...
            var currentInventoryEquipments = currentInventoryProps.OfType<Equipment>()
                                                                  .ToArray();
            ...
            for (var slotIndex = 0; slotIndex < equipmentCarrier.Slots.Length; slotIndex++)
            {
                ...
                if (equiped == null)
                {
                    var availableEquipments = currentInventoryEquipments
                                              .Where(equipment =>
                                                  (equipment.Scheme.Equip.SlotTypes[0] & slot.Types) > 0)
                                              .Where(equipment =>
                                                  EquipmentCarrierHelper.CanBeEquiped(equipmentCarrier, slotIndex,
                                                      equipment))
                                              .ToArray();

...
namespace Zilon.Core.Benchmarks
{
    ...
    {
        public Config(string buildNumber, int iterationCount, string monoRuntimePath, string benchArtifactsPath, string schemePath)
        {
            ...
            Add(Job.Default.With(Runtime.Core).With(Platform.X64).With(Jit.RyuJit).WithIterationCount(iterationCount).WithEnvironmentVariable("zilon_SchemeCatalog", schemePath));

namespace Zilon.Core.Common
{
    public static class HexHelper
    {
        ...
        {
            CubeCoords[] offsets = GetOffsetClockwise();
            CubeCoords[] neighborCoords = new CubeCoords[6];
            for (var i = 0; i < 6; i++)
            {
                CubeCoords offset = offsets[i];
        ...
        {
            CubeCoords baseCubeCoords = ConvertToCube(baseX, baseY);

            CubeCoords[] neighborCoords = GetNeighbors(baseCubeCoords);
